"""
Direct image model training script — bypasses notebook runner issues.
Trains EfficientNet-B0 (best benchmark model) on the full training set.
Saves all artifacts required by NB05 and the dashboard.
"""
...
print("="*70)
print("FOREST SENTINEL — IMAGE MODEL TRAINING")
print("="*70)
...
with open(META_DIR / "selected_model.json", "w") as f:
    json.dump({
        ...
        "selected_by": "benchmark_f1",
        "benchmark_f1": 0.9850,
        "benchmark_acc": 0.9850,
        "inference_ms": 13.38
    }, f, indent=2)
...
class FireDataset(Dataset):
    def __init__(self, items, transform, class_to_idx=None):
        # Items can be [path, class_name] lists or {"path":..., "label":...} dicts
...
# ─── Model ───────────────────────────────────────────────────────────────────
def build_efficientnet_b0(nc=2):
...
model = build_efficientnet_b0(nc=2)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_f1 > best_f1:
        ...
        best_epoch = epoch
        no_improve = 0
        torch.save(model.state_dict(), IMG_DIR / "best_model.pth")
...
print(f"\nTraining complete. Best epoch: {best_epoch}, Best Val F1: {best_f1:.4f}")
...
plt.suptitle(f'EfficientNet-B0 Training — Best Val F1={best_f1:.4f}',
...
# Load best model
model.load_state_dict(torch.load(IMG_DIR / "best_model.pth", map_location='cpu'))
model.eval()
